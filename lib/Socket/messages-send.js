...
while (inner && typeof inner === "object") {
  ...
  if (
    ...
    hasSpecial = true;
    if (value.interactiveMessage?.nativeFlowMessage?.buttons?.length) {
      interactiveButtons = value.interactiveMessage.nativeFlowMessage.buttons;
    }
  }
...
if (hasWrapper || hasSpecial) {
  const flowNames = interactiveButtons && interactiveButtons.length
    ? [...new Set(interactiveButtons.map((b) => b.name || 'quick_reply'))]
    : ['quick_reply'];
  stanza.content.push({
    tag: 'biz',
    attrs: {},
